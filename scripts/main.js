...
const btnNavSeleccionArgentina = document.querySelector(
  "#btnNavSeleccionArgentina"
);
...
searchInput.value = "";
const productNotFoundedAlert = document.querySelector(
  "#productNotFoundedAlert"
);

const mainInformation = document.querySelector("#mainInformation");

const productsInformationTitle = document.querySelector(
  ".productsInformation__title"
);
const productsInformationResults = document.querySelector(
...
const templateContainerProducts = document.getElementById(
  "template-containerProducts"
).content; //template para cargar los items en contenedor
...
const templateCartProducts = document.getElementById(
  "template-CartProducts"
).content; //template para cargar los items en el carrito
const fragmentCart = document.createDocumentFragment(); //fragmento para guardar cada item y luego insertarlo en el carrito
const cartResumeTotal = document.querySelector(
  ".cart__sideContainer__resume__total"
);
const btnFinishBuy = document.querySelector(
  ".cart__sideContainer__resume__btnFinishBuy"
);
const productAddToast = document.querySelector(".productAddToast");
const cartProductsCounter = document.querySelector(
  ".navBar__menu__btnCartMenu-counter"
);
const cartProductsCounterResponsive = document.querySelector(
  ".navBar__btnCartResponsive-counter"
);

const btnFooterLigaProfesional = document.querySelector(
  "#btnFooterLigaProfesional"
);
const btnFooterPrimeraNacional = document.querySelector(
  "#btnFooterPrimeraNacional"
);
const btnFooterTodosProductos = document.querySelector(
  "#btnFooterTodosProductos"
);
const btnFooterSeleccionArgentina = document.querySelector(
  "#btnFooterSeleccionArgentina"
);

document.addEventListener("DOMContentLoaded", () => {
  //Despues de cargarse el DOM
  fetchData();
...
const loadContainerProducts = (data) => {
  ...
  data.forEach((product) => {
    templateContainerProducts
      .querySelectorAll("img")[0]
      .setAttribute("src", `./assets/images/${product.imgFrontUrl}`);
    templateContainerProducts
      .querySelectorAll("img")[0]
      .setAttribute("alt", `${product.name}`);
    templateContainerProducts
      .querySelectorAll("img")[1]
      .setAttribute("src", `./assets/images/${product.imgBackUrl}`);
    templateContainerProducts
      .querySelectorAll("img")[1]
      .setAttribute("alt", `${product.name}`);
    templateContainerProducts.querySelector(
      "h3"
    ).textContent = `${product.name} ${product.year}`;
    templateContainerProducts.querySelector(
      "p"
    ).textContent = `$${product.price}`;
    templateContainerProducts.querySelector(
      ".productsContainer__item__button"
    ).dataset.id = product.id; //guardo en el button el id de ese producto
...
const setCart = (object) => {
  ...
  if (cart[product.id]) {
    //producto cargado, le aumento una unidad
...
function updateCartProductView() {
  ...
  cart.forEach((product) => {
    if (product) {
      //si no es null
      templateCartProducts.querySelector(
        ".cart__sideContainer__items__item__main__name"
      ).textContent = product.name;
      templateCartProducts
        .querySelector("img")
        .setAttribute("src", `${product.img}`);
      templateCartProducts.querySelector(
        ".cart__sideContainer__items__item__description__units__number"
      ).textContent = product.units;
      templateCartProducts.querySelector(
        ".cart__sideContainer__items__item__description__price"
      ).textContent = `$${Number(product.price.slice(1)) * product.units}`;
      templateCartProducts.querySelector(
...
searchInput.addEventListener("keyup", (e) => {
  if (searchInput.value == 0) {
    if (productNotFoundedAlert.classList.contains("show"))
      //si el input está vacio, quita el cartel no encontrado
...
const searchProduct = (searchInput) => {
  ...
  if (result.length == 0) {
    ...
    if (nav__menu.classList.contains("show")) {
      //En dispositivo movil, cierra el menu del nav
      nav__menu.classList.toggle("show");
      btn__nav.classList.toggle("active");
    }
    if (productNotFoundedAlert.classList.contains("show"))
      //oculta el cartel de no encontrado, si estaba visible
      productNotFoundedAlert.classList.toggle("show");
...
navLogo.addEventListener("click", (e) => {
  ...
  if (cartView.classList.contains("show")) cartView.classList.toggle("show");
...
const arrayLength = (array) => {
  ...
  array.forEach((product) => {
    if (product) count += product.units;
  });
  return count;
};
...
const addEventShowCart = () => {
  ...
  const btnCloseCartView = document.querySelector(
  ...
  const btnCartViewResponsive = document.querySelector(
    "#btnCartViewResponsive"
  );
